...
const App = () => {
  ...
  axios.get("http://localhost:3001/requests/").then(
      (response) => {
       let myReq = response.data.filter(req => {
         console.log(req.id)
         const volunteerList = axios.get("http://localhost:3001/requests_users")
         .then(resp => {
          //  console.log(resp.data)
        //  console.log(req);
           const vol = resp.data.filter((vol) => req.id === vol.request_id);
          //  return vol.length < 4;
           console.log(vol.length < 4;
    ...
    //   let allReq = response.data.filter((request) => {
