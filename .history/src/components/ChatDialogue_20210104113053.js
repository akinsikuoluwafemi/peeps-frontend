...
let {allRooms} = useContext(AllRoomContext)    
  ...
  const roomsToShowAfterFulfilled = () => {
    ...
    let allReq = successRepReq.map((req) => {
  ...
  const SuccessAndRepublish = async (id) => {
    const token = JSON.parse(localStorage.getItem("token"));

    let res = await axios
      .get(`http://localhost:3001/successandrepublish/${id}`, {
        headers: {
      ...
      .then(
        (response) => {
          ...
          setSuccessRepReq(response.data);
