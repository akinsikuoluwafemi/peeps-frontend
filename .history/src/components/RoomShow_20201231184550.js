...
import { RoomDataContext,UserIdContext,ChatRoomIdContext, ReqOwnerFirstNameContext, SelectedRequestContext, CurrentVolunteerContext} from '../ContextFile';
...
let roomParam = currentRoom.room.id || parseInt(loctaion.pathname.match(/\d+$/)[0]);
  ...
  const handleChecked = (e) => {
    ...
    if (checked) {
      alert("setting fulfilled to false");
    } else if (!checked) {
      alert("setting fulfilled to true");
    }
  }
