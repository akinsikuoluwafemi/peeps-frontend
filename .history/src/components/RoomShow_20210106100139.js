...
import { RoomDataContext,UserIdContext,ChatRoomIdContext, ReqOwnerFirstNameContext, SelectedRequestContext, CurrentVolunteerContext, DeactivateContext,
RequestOwnerContext
} from '../ContextFile';
...
export default function RoomShow({cableApp}) {
  ...
  let {requestOwner, setRequestOwner} = useContext(RequestOwnerContext)
  ...
  let [ checked, setChecked ] = useState(currentVol.fulfilled || null);
...
let roomParam = currentRoom.room.id || parseInt(loctaion.pathname.match(/\d+$/)[0]);
  ...
  const handleChecked = (e) => {
    ...
    // alterFulfilled(requestOwner || userId || currentVol.id);
