...
export default function RoomShow({cableApp}) {
  ...
  let { requiresRepublishing } = useContext(
    RepublishingContext
  );
...
const checkActiveRequest = async (id) => {
  ...
  let res = await axios
    .get(`https://peeps-platform.herokuapp.com/deactivate/${id}`, {
      headers: {
        Authorization: `Basic ${token}`,
      },
    })
...
console.log(currentVol.request_id)
  ...
  const deactivateRooms = async (id) => {
    ...
    let res = await axios
      .get(`https://peeps-platform.herokuapp.com/deactivaterooms/${id}`,
        {
         headers: {
           Authorization: `Basic ${token}`,
         },
  ...
  deactivateRooms(currentVol.request_id)
   ...
   const alterFulfilled = (id) => {
    ...
    const token = JSON.parse(localStorage.getItem("token"));
     ...
     let res = axios
       .patch(`https://peeps-platform.herokuapp.com/requests_users/${id}`, obj, {
         headers: {
           Authorization: `Basic ${token}`,
         },
       })
    ...
    <>
      {requiresRepublishing ? (
        <div className="text-center m-auto">
          ...
          <Link onClick={() => window.location.reload()} to="/feed">
            Go to Feeds
          </Link>
        </div>
      ) : (
