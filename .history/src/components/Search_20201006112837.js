...
export const Map = () => {
  ...
  const RemoveRedundantRequest = () => {
    const volunteerList = allVolunteers.filter((item,i) => {
      
      console.log(i)
      let arr = []
      let numArr = arr.push(item.request_id);
      // let num = [item.request_id]
      // console.log([...new Set(numArr)]);
      // console.log([...new Se(numArr);

      return item.request_id.split;
