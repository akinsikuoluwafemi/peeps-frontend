...
export const Map = () => {
  ...
  //   console.log('I just volunteered for request' + requestId + 'the owner is ' + requestOwner)
  ...
  const onCreateRoom = async () => {
    ...
    let res =  await axios.post("http://localhost:3001/rooms", {
        roomObj
  ...
  const onVolunteerClick = async () => {
    ...
    axios.all([
     ...
     await axios.post("http://localhost:3001/requests_users", {
        volunteer
      }, {
        'Authorization': `Basic ${userData.token}`,
  ...
  console.log('Date created: ', responseArr[0].data.created_at + ' ' + responseArr[0].data);
  console.log('Date created: ', responseArr[1].data.created_at + ' ' + responseArr[0].data);
}, (error) => {
  console.log(error)
})
