...
export const Map = () =>
{
  ...
  let [previouslyVolunteered, setPreviouslyVolunteered] = useState(null);
  let [clickCount, setClickCount] = useState(null);
  ...
  const onVolunteerClick = async (user_id, request_id) => {
    console.log(
      "I just volunteered for request" +
        requestId +
        "the owner is " +
  ...
  const getUserclickedRequest = () => {
    let userReq = allVolunteers.filter((user) => {
  ...
  const checkedClickRequest = () => {
    let userclickedReq = userRequest.filter(volunteer => {
      
      let req = allRequest.filter(request => {
        return  volunteer.request_id === request.id;
      })
      console.log(req);
  ...
  return (
    <div>
      ...
      >
        ...
        {selectedRequest && (
          <InfoWindow
            onDomReady={checkedClickRequest}
            position={{
