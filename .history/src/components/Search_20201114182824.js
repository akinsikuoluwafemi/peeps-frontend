...
export const Map = () =>
{
  ...
  let [addOneToCount, setAddOneToCount] = useState(0)
  ...
  const updateClickCount = async ( rid) => {
    ...
    const data = {
     click_count: setAddOneToCount(addOneToCount++)
    };
    

    // localStorage.setItem('click_count', addOneToCount)
        let add = setAddOneToCount(addOneToCount + 1)
    let add = 
    const token = JSON.parse(localStorage.getItem("token"));
    console.log(token);
    const res = await axios
      .patch(`http://localhost:3001/requests/${rid}`, {
        click_count: setAddOneToCount(addOneToCount + 1)
      }, {
        headers: {
         Authorization: `Basic ${token}`,
        },
