...
const checkSameUserClick = async (id) => {
  ...
  getVolunteeredRoomId(reqDescription);
  alert(`The request owner id is: ${requestOwner}`)
     return res;
...
const checkFulfilledRequest = async (id) => {
  ...
  const getVolunteeredRoomId = async (desc) => {

    const token = JSON.parse(localStorage.getItem("token"));

  let res = await axios
    .get(`http://localhost:3001/volunteeredroomid/${desc}`, {
      headers: {
        Authorization: `Basic ${token}`,
      },
    })
  ...
  // getVolunteeredRoomId('I need help to fix some plumbing issues in my apartment')
...
const renderButton = () => {
    if(userId === requestOwner){
      ...
      return (
        <>
          <p className="badge h3 badge-info mr-3">Already Volunteered</p>
          {/* <Link to="/rooms/29">Go to room</Link> */}
        </>
      );
