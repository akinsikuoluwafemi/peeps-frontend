...
export const Map = () =>{
 ...
 const mapRef = useRef();
  ...
  const onRequestRoomCreate = async (chat) => {
    ...
    alert(`roomid: ${chatRoomId}, request_id: ${requestId}`)

    const token = JSON.parse(localStorage.getItem("token"));

    const res = await axios
      .post("http://localhost:3001/requests_rooms", data, {
        headers: {
          Authorization: `Basic ${token}`,
        },
      })
          ...
          {selectedRequest && (
            ...
            >
              <div>
                ...
                {reqOwnerFirstName && <p>Name: {reqOwnerFirstName}</p>}
...
function AddRequest({ panTo })
{
  ...
  const { showRequestForm } = useContext(RequestFormContext)
  ...
  return (
    <div className={showRequestForm ? `center` : `remove-search`}>
